...
public class OrganizerController
{
	...
	public String delete( @PathVariable( "id" ) Long organizerId, final RedirectAttributes redirectAttributes, Model model )
	{
		OrganizerDto organizer = organizerService.findOrganizer( organizerId );
		...
		final RedirectAttributes redirectAttributes, Model model )
	{
		...
		{
			model.addAttribute( Messages.ATTRIBUTE_NAME, Messages.createWithError( MessageKeyEnum.ORGANIZER_SAVE_INVALID ) );
		...
		final RedirectAttributes redirectAttributes, Model model )
	{
		if ( bindingResult.hasErrors() )
		{
			model.addAttribute( Messages.ATTRIBUTE_NAME, Messages.createWithError( MessageKeyEnum.ORGANIZER_UPDATE_INVALID ) );
	...
	{
		response.sendError( HttpStatus.NOT_FOUND.value(), String.format( "Organizer %s does not exist.", ex.getMessage() ) );
	}
}
